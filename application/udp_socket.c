...
static void run_server(const char *local_ip, unsigned short port)
{
	...
	while (1) {
		conn = udp_socket_accept_connection(local_ip, port);
		if (!conn) {
		...
		while (1) {
			len = udp_socket_recv(conn, buf, 127);
			if (len <= 0) {
...
{
	struct udp_socket_conn *conn;
	char *str = "this is a udp socket tx/tx test string\n";
	int len;

	conn = udp_socket_connect(target_ip, port);
	if (!conn) {
		dbg_printf("connect failed\n");
		return;
	}

	len = udp_socket_send(conn, str, strlen(str));
	if (len <= 0)
