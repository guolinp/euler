...
static void run_server(const char *pathname)
{
	...
	while (1) {
		conn = serial_accept_connection(pathname);
		if (!conn) {
...
{
	struct serial_conn *conn;
	char *str = "this is a serial tx/tx test string\n";
	int len;

	conn = serial_connect(pathname);
	if (!conn) {
		dbg_printf("connect failed\n");
		return;
	}

	len = serial_send(conn, str, strlen(str));
	if (len <= 0)
...
int main(int argc, char *argv[])
{
	if (argc >= 3) {
		...
		fprintf(stderr, "Usage: %s <server|client> <serial_file_pathname>\n", argv[0]);
	}
